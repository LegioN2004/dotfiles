...
then
 apt update -y && apt upgrade -y && apt-get install amd64-microcode nala fish alacritty build-essential python3-pip xorg i3 i3-wm i3blocks i3lock i3pystatus i3status xinput git light lightdm lightdm-gtk-greeter gcc pulseaudio-utils playerctl stow neovim tmux libx11-dev lm-sensors libxinerama-dev libxft-dev 
 chmod +s /usr/bin/light
...
then
 ...
 yes|pacman -Syyu neofetch htop flameshot redshift xclip maim vim lxappearance ranger papirus-icon-theme noto-fonts-emoji ttf-font-awesome jdk-openjdk exa ttf-ubuntu-mono-nerd bluez blueman ripgrep
...
cp -r "/home/$username/dotfiles/scripts-laptop/keyd config/default.conf"  /etc/keyd/default.conf
sudo keyd reload
cd
cd
...
cd /home/$username/dotfiles/ && git checkout main
cd
apt autoremove
exit
